...
func TestCollectionWindowKeyPreservesUserAndFiltersPagination(t *testing.T) {
	base := url.Values{"includeMeta": {"1"}, "X-Plex-Container-Start": {"12"}, "X-Plex-Container-Size": {"24"}, "X-Plex-Token": {"secret"}}
	otherPage := url.Values{"includeMeta": {"1"}, "X-Plex-Container-Start": {"36"}, "X-Plex-Container-Size": {"39"}}
	...
	if a != b {
		t.Fatal("ranges for one user must share their collection window")
	}
